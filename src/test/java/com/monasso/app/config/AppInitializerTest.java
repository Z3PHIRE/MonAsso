...
class AppInitializerTest {
    ...
    @Test
    void shouldExposeClearErrorWhenDatabaseIsCorrupted() throws Exception {
        Path tempHome = Files.createTempDirectory("monasso-appinit-corrupt");
        withTemporaryHome(tempHome, () -> {
            Files.createDirectories(AppPaths.dataDir());
            Files.writeString(AppPaths.databasePath(), "contenu invalide");
            ...
            assertTrue(
                    exception.getMessage().toLowerCase().contains("corrompue")
                            || exception.getMessage().toLowerCase().contains("invalide")
            );
        });
    }

    private void withTemporaryHome(Path homePath, CheckedRunnable runnable) throws Exception {
        String previousHome = System.getProperty("monasso.home");
        System.setProperty("monasso.home", homePath.toString());
        try {
            runnable.run();
        } finally {
